...
func (d *db) FindOne(ctx context.Context, id string) (u user.User, err error) {
	...
	if err = result.Decode(&u); err != nil {
		return u, fmt.Errorf("failed to find one user by id: %s from DB due to error: %v", id, err)
	}
	return u, nil
}
...
func (d *db) Update(ctx context.Context, user user.User) error {
	...
	if err != nil {
		return fmt.Errorf("failed to unmarhsal user. error: %v", err)
	}
	...
	d.logger.Tracef("Mathched %d documets and Modified %d documents", result.MatchedCount, result.ModifiedCount)
